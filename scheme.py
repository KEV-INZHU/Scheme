...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        new_call_frame = Frame(self)
        while formals != nil or vals != nil:
            new_call_frame.bindings[formals.first] = vals.first
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        defined = scheme_eval(expressions.rest.first,env)
        env.define(target, defined)
        return target
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    names = bindings.map(lambda a: a.first)
    bindings.map(lambda a: validate_form(a, 2, 2))
    values  = bindings.map(lambda a: scheme_eval(a.rest.first, env))

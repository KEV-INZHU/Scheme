...
(define (zip pairs)
  (cond 
    ...
    (else (begin (define next (zip (cdr pairs)))
                 (define first (cons (caar pairs) (car next)))
                 (define second (cons (cons (cadr (car pairs)) (cadr next)) nil))
                 (cons first second)))))
...
(define (nondecreaselist s)
  ...
  (cond 
    ...
    (else (let ((next (nondecreaselist (cdr s)))) (cons (cons (car s) (car next)) (cdr next))))
  )
)
    ; END PROBLEM 17
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (define nums (let-to-lambda (car (cdr (zip values)))))
           (define eval (let-to-lambda body))
           (cons (cons 'lambda (cons params eval)) nums)
           ; END PROBLEM EC
           ))
        (else
         ; BEGIN PROBLEM EC
